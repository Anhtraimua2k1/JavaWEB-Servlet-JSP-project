...
// take cookies to cart
        ...
        for (int i=0;i< carts.size();i++) {
            ...
            if (cart.getProduct().getId() == id) {
                ...
                int total = quantity + cart.getQuantity();
                if (total > 0) {
...
// not have 
        if (false == checkProduct) {
            if (quantity > 0) {
                Cart cart = new Cart();
                cart.setProduct(productDB.getProductsById(id));
                cart.setQuantity(quantity);
                carts.add(cart);
            }
        ...
        // 
        // if ()         Cookie c =new Cookie(id,"quanty);
...
//        ArrayList<Cart> n = (ArrayList<Cart> )request.getSession().getAttribute("listCarts");
//        out.println("" +  n.get(0).getProduct().getName());
    }
